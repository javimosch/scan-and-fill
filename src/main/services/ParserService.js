...
export default class ParserService {
   ...
   * Uses pdf-parse metadata when available, otherwise parses the PDF
  ...
  async getPageCount(filePath, pdfData = null) {
    try {
      ...
      if (pdfData && (pdfData.numpages || pdfData.numPages || pdfData.info?.Pages)) {
        return pdfData.numpages || pdfData.numPages || pdfData.info.Pages;
      }

      const dataBuffer = fs.readFileSync(filePath);
      const data = await pdfParse(dataBuffer);

      if (data && (data.numpages || data.numPages || data.info?.Pages)) {
        return data.numpages || data.numPages || data.info.Pages;
      }
