...
import { Settings, X, Key, Brain, Database, Trash2, FileText } from 'lucide-react';
...
export default function SettingsModal({ isOpen, onClose }) {
  ...
  const clearOCRCache = async () => {
    if (confirm('Are you sure you want to clear the OCR cache? This will force fresh OCR extraction on next analysis.')) {
      try {
        setLoading(true)
      ...
      <div className="modal-content" style={{
            ...
            <div style={{ 
              ...
              {cacheStats.lastUpdated && (
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontWeight: '500' }}>{new Date(cacheStats.lastUpdated).toLocaleDateString()}</span>
                </div>
              )}
            </div>
          )}
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
          ...
          >
            {loading ? (
              <>
                <div style={{ width: '16px', height: '16px', border: '2px solid var(--text)', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
                Saving...
              </>
            ) : (
              'Save Settings'
